'use strict'
{
  ...
  let intervalControlNumber = setInterval(() => {
    if (Date.now() > parseAfter) {
      ...
      // on what changed
      if (
        lastSpacedMode !== spacedModeInput.checked ||
        fieldInfix.value !== lastInfix
      ) {
        lastInfix = fieldInfix.value
        clearFieldBorders()
        fieldInfix.classList.add('border-primary')
        let optionalResults = {}
        let postfix = toPostfix(
          fieldInfix.value,
          spacedModeInput.checked,
          optionalResults
        )
...
function toPostfix(infixExpression, spacedMode = false, optionalResults = {}) {
  ...
  toPostfixI(funcObj)
  if (optionalResults.status === 'error') return optionalResults
...
function toPostfixI(fo, depth = 0) {
    ...
    if (mode === 0) {
      ...
      if (to === ')') {
        if (depth === 0) {
          ...
          postfixCheckWarnings(
            fo,
            operatorStack,
            lastClauseMinimumPrecedence,
            precedence
          )
        ...
        fo.optionalResults.status = 'error'
        if (to === '(')
          fo.optionalResults.message = 'Unexpected opening parenthesis'
  ...
  if (depth !== 0) {
    ...
    fo.optionalResults.status = 'error'
    fo.optionalResults.message =
      'End of input with missing ' + 'closing parenthesis'
    fo.optionalResults.position = fo.i
    return -2
  }
...
function postfixCheckWarnings(
  ...
  if (operatorStack.isEmpty()) {
    ...
    if (
      lastClauseMinimumPrecedence === lastOperator.precedence &&
      lastClauseMinimumPrecedence >= precedence &&
      lastOperator.isCommutative
    ) {
      if (fo.optionalResults.status === 'success')
        fo.optionalResults.status = 'log'
...
function addOperatorStackToOutput(fo, operatorStack, precedence = -1) {
  while (
    !operatorStack.isEmpty() &&
    operatorStack.peek().precedence >= precedence
  )
...
function addToOutput(fo, toAdd, outputType) {
  if (
    !fo.spacedMode ||
    fo.lastOutputType === 'start' ||
    !(fo.lastOutputType === 'operand' && fo.lastOutputType === outputType)
  )
    fo.output += toAdd
